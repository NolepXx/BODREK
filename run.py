...
/____/\____/____/_/|_/___/_/|_|  
 _______   ___   _   ___  ___  ________ _     
/  __ \ \ / / \ | | / _ \ |  \/  |  _  | |    
| /  \/\ V /|  \| |/ /_\ \| .  . | | | | |  :  ℕ𝕆𝕃𝔼ℙ𝕏𝕏𝕏
| |     \ / | . ` ||  _  || |\/| | | | | |    
| \__/\ | | | |\  || | | || |  | \ \_/ / |____
...
def menu(id):
	...
	banner()
	print(f'{ung}─────────────────────────────')
	print(f'{biru}━─═ ◕➤ketik y untuk mulai Crack Publik massal ketik yes')
...
def nge_crack():
	try:
		...
		exit()
	try:
		jum = int(input(f'{biru}âââ ââ¤ Mau Berapa Idz Target  : '))
	except ValueError:
	...
	for met in range(jum):
		yz+=1
		kl = input(f'{biru}âââ ââ¤ Masukan Idz Target Yang Ke '+str(yz)+' : ')
		uid.append(kl)
	for userr in uid:
		try:
			...
			exit()
	try:
		print(f'{biru}âââ ââ¤ Total Idz Target Yang Terkumpul : '+str(len(id)))
		atur_dulu()
	except requests.exceptions.ConnectionError:
		print(f'{x}')
		print('{biru}âââ ââ¤ Unstable Signal ')
